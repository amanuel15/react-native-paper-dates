...
function DatePickerModalContentHeader(props) {
  ...
  const supportingTextColor = _utils.passedTheme.isV3 ? _utils.passedTheme.colors.onSurfaceVariant : color;
  const textFont = _utils.passedTheme !== null && _utils.passedTheme !== void 0 && _utils.passedTheme.isV3 ? _utils.passedTheme.fonts.labelMedium : _utils.passedTheme.fonts.medium;
  return /*#__PURE__*/React.createElement(_reactNative.View, {
    ...
    icon: collapsed ? editIcon ?? _utils.passedTheme.isV3 ? 'pencil-outline' : 'pencil' : calendarIcon ?? _utils.passedTheme.isV3 ? 'calendar-blank' : 'calendar',
    ...
    iconColor: _utils.passedTheme.isV3 ? _utils.passedTheme.colors.onSurface : color,
    onPress: onToggle
  }) : null);
}
function HeaderContentSingle(_ref) {
  ...
  const dateColor = state.date ? _utils.passedTheme.isV3 ? _utils.passedTheme.colors.onSurface : color : lighterColor;
...
function HeaderContentMulti(_ref2) {
  ...
  const lighterColor = (0, _color.default)(color).fade(0.5).rgb().toString();
  const dateColor = dateCount ? _utils.passedTheme.isV3 ? _utils.passedTheme.colors.onSurface : color : lighterColor;
  const formatter = React.useMemo(() => {
    return new Intl.DateTimeFormat(locale, {
      month: 'short',
      day: 'numeric'
...
function HeaderContentRange(_ref3) {
  ...
  const lighterColor = (0, _color.default)(color).fade(0.5).rgb().toString();
  const startColor = state.startDate ? _utils.passedTheme.isV3 ? _utils.passedTheme.colors.onSurface : color : lighterColor;
  const endColor = state.endDate ? _utils.passedTheme.isV3 ? _utils.passedTheme.colors.onSurface : color : lighterColor;
