...
export default function DatePickerModalContentHeader(props) {
  ...
  const textFont = passedTheme !== null && passedTheme !== void 0 && passedTheme.isV3 ? passedTheme.fonts.labelMedium : passedTheme.fonts.medium;
  return /*#__PURE__*/React.createElement(View, {
    ...
    icon: collapsed ? editIcon ?? passedTheme.isV3 ? 'pencil-outline' : 'pencil' : calendarIcon ?? passedTheme.isV3 ? 'calendar-blank' : 'calendar',
...
export function HeaderContentSingle(_ref) {
  ...
  const dateColor = state.date ? passedTheme.isV3 ? passedTheme.colors.onSurface : color : lighterColor;
...
export function HeaderContentMulti(_ref2) {
  ...
  const lighterColor = Color(color).fade(0.5).rgb().toString();
  const dateColor = dateCount ? passedTheme.isV3 ? passedTheme.colors.onSurface : color : lighterColor;
  const formatter = React.useMemo(() => {
    return new Intl.DateTimeFormat(locale, {
      month: 'short',
      day: 'numeric'
...
export function HeaderContentRange(_ref3) {
  ...
  const lighterColor = Color(color).fade(0.5).rgb().toString();
  const startColor = state.startDate ? passedTheme.isV3 ? passedTheme.colors.onSurface : color : lighterColor;
  const endColor = state.endDate ? passedTheme.isV3 ? passedTheme.colors.onSurface : color : lighterColor;
